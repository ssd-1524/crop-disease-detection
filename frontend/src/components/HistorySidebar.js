...
export default function HistorySidebar() {
  ...
  // Stable reference — prevents useCallback/useEffect from re-running on every render
                  ...
                  <div className="flex items-center p-2 rounded-lg hover:bg-gray-100 transition-colors cursor-pointer">
                    <div className="flex items-center gap-2 mr-4 flex-shrink-0">
                      {item.signedImageUrl ? (
                        ...
                        <div className="w-16 h-16 rounded-md bg-gray-200 flex items-center justify-center text-xs text-center">
                          No Image
                        </div>
                      )}
                      {item.sam_mask_image && (
                        <img
                          src={`data:image/jpeg;base64,${item.sam_mask_image}`}
                          alt="Disease overlay"
                          className="w-16 h-16 rounded-md object-cover ring-1 ring-red-200"
                        />
                      )}
                    </div>
